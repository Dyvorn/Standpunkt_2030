...
def handle_answer(data):
    ...
    if engine.submit_answer(request.sid, idx):
        ...
        print(f"Antwort abgelehnt für {request.sid}. Grund: Falsche Phase oder SID nicht in engine.players (Bekannt: {request.sid in engine.players})")
...
def handle_state_request():
    ...
    if q and engine.state == "QUESTION":
        ...
        if not engine.is_reading_time:
            emit('start_voting', room=request.sid)
